...
untouched and still needs the human rewrite pass described in TODO.md.
...
And the reverse for any review page that's INDEXABLE but was previously
marked noindex / dropped from the sitemap (idempotent in both directions).
...
def main():
    ...
    removed, kept_review_urls = [], 0
    def strip_block(m):
        nonlocal removed, kept_review_urls
        block = m.group(0)
    ...
    print(f"\nsitemap: removing {len(removed)} review URL(s); {kept_review_urls} review URL(s) remain")
    if removed[:10]:
        print("  e.g.:", removed[:10])
    ...
    expected = {"PASS": 18, "PASS_WITH_WARNING": 104, "PASS_WITH_PROMINENT_WARNING": 16}
    ok = all(verdicts.get(k, 0) == v for k, v in expected.items()) and verdicts.get("FAIL", 0) == 0
    print("MATCHES EXPECTED (18/104/16/0):", ok)
